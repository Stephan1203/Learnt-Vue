      ...
      <div v-if="todo_list.length > 0">
        <ul>
          <li v-for="todo in todo_list" :key="todo.id">
            <p class="todo_info">{{ todo.id }}. {{ todo.userName }} {{ todo.date }}</p>
            <p class="todo_item">{{ todo.todo }}</p>
          </li>
        </ul>
        <svg @click="delete_list" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 110.61 122.88" height="30px" width="30px">
